...
public class Main11659 {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<m;i++) {
			...
			System.out.println(sum(0,n-1,1,st-1,end-1));
		}
		
	}
	private static int sum(int start, int end, int now, int left, int right) {
		if(left>end||right<start)return 0;
		if(left<=start&&end<=right)return tree[now];
		int mid=(start+end)/2;
		return sum(start,mid,now*2,left,right)+sum(mid+1,end,now*2+1,left,right);
	}
	private static int init(int start, int end, int now) {
		if(start==end) {
			return tree[now]=num[start];
		}
		int mid=(start+end)/2;
		return tree[now]=init(start,mid,now*2)+init(mid+1,end,now*2+1);
	}
}
